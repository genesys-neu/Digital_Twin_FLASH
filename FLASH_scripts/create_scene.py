...
def make_env_cat_1(header, row):
    ...
    for car in car_objects:
        ...
        if obj is not None:
            ...
            for index, header_value in enumerate(header):
                if car in header_value:
                    if car_header_index is None:
                        car_header_index = int(index)
                    elif car_loc_x is None:
                        car_loc_x = int(index)
                    elif car_loc_y is None:
                        car_loc_y = int(index)
                if car in header_value and "Motion" in header_value:
                    car_movement_index = int(index)
                    # print(row[car_movement_index])
                    break
            
            # print(row[car_header_index])
            # print(f"CAR {car} LOCATION: ({row[car_loc_x]}, {row[car_loc_y]})")
            ...
            if row[car_header_index] == 'Y':
                print(type(row[car_loc_x]))
                obj.location.x = float(row[car_loc_x])
                obj.location.y = float(row[car_loc_y])
                obj.location.z = 0.5

            elif row[car_header_index] == 'N':
                obj.location.z = -100
            ...
            if row[car_header_index] == 'Y':
                ...
                obj.location.y = float(row[car_loc_y])
                obj.location.z = 0.5
                if car_movement_index is not None and row[car_movement_index] == 'Y':
                    if car == "Car.002":
                        obj.location.x += -72
                        obj.location.y += -62
                    elif car == "Car.004":
    ...
    if "blender3.3.8" in (bpy.app.binary_path):
        row_file_path = os.path.abspath(f'blend_files/blend_files_camera/category_1/ep_' + row[0] + '.blend')
    elif "blensor" in (bpy.app.binary_path):
        row_file_path = os.path.abspath(f'blend_files/blend_files_blensor/category_1/ep_' + row[0] + '.blend')
    bpy.ops.wm.save_as_mainfile(filepath=row_file_path)


def make_env_cat_2(header, row):
    ...
    for case in pedestrian_pos_list:
        ...
        if "blender3.3.8" in (bpy.app.binary_path):
            row_file_path = os.path.abspath(f'blend_files/blend_files_camera/category_2/{case[2]}/ep_{row[0]}.blend')
        elif "blensor" in (bpy.app.binary_path):
            row_file_path = os.path.abspath(f'blend_files/blend_files_blensor/category_2/{case[2]}/ep_{row[0]}.blend')
        bpy.ops.wm.save_as_mainfile(filepath=row_file_path)

def make_env_cat_3(header, row):
    ...
    # Save the changes
    if "blender3.3.8" in (bpy.app.binary_path):
        row_file_path = os.path.abspath(f'blend_files/blend_files_camera/category_3/ep_{row[0]}.blend')
    elif "blensor" in (bpy.app.binary_path):
        row_file_path = os.path.abspath(f'blend_files/blend_files_blensor/category_3/ep_{row[0]}.blend')
    bpy.ops.wm.save_as_mainfile(filepath=row_file_path)

def make_env_cat_4(header, row):
    ...
    for case in ferrari_loc_list:
        ...
        if "blender3.3.8" in (bpy.app.binary_path):
            row_file_path = os.path.abspath(f'blend_files/blend_files_camera/category_4/{case}/ep_{row[0]}.blend')
        elif "blensor" in (bpy.app.binary_path):
            row_file_path = os.path.abspath(f'blend_files/blend_files_blensor/category_4/{case}/ep_{row[0]}.blend')
        bpy.ops.wm.save_as_mainfile(filepath=row_file_path)
...
if __name__ == '__main__':
    ...
    # print(sys.argv[-1])
    # print(sys.argv[-2])
    header = eval(sys.argv[-2])
    print(header)
    row = eval(sys.argv[-1])
    print(row[0])
    make_env_cat_1(header, row)
    make_env_cat_2(header, row)
    make_env_cat_3(header, row)
    make_env_cat_4(header, row)