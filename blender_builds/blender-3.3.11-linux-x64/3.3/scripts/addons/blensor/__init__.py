...
def velodyne_layout(obj, layout):
            ...
            row = layout.row()
            row.operator("blensor.randomize_distance_bias", text="Randomize bias")        
            row = layout.row()
            col = row.column()
    ...
    def draw(self, context):
        ...
        try:
            if obj.type == "CAMERA":
                status = obj.name
                is_cam = True
        except bpy.utils.errors.MissingOperatorException as e:
            print("Error: ", e)
        ...
        if is_cam:
            ...
            row = layout.row()
            row.operator("blensor.scan", text="Single scan")        
            row = layout.row()
            row.operator("blensor.scanrange", text="Scan range")        
            row = layout.row()
            col = row.column()
            col.operator("blensor.export_motion", text="Export motion")
            col = row.column()
            col.operator("blensor.delete_scans", text="Delete scans")
...
class OBJECT_OT_scanrange_handler(bpy.types.Operator):
    ...
    name="Start Frame",
    description="The starting frame for the scan",
    default=1,
    )
    _timer = None   
